...
impl EdgeGroupAndOrientMoveTable {
    fn get_slice_for_coords(&self, group: EdgeGroupCoord, orient: EdgeOrientCoord) -> &[u16; 66] {
        ...
        as_u16_slice(&self.0)[66 * i..66 * (i + 1)].as_array().unwrap()
    }
    ...
    pub fn conjugate_by_transform(
        ...
        (slice[transform_index].into(), slice[transform_index + 1].into())
    }
    ...
    pub fn get_sym_representative(
        ...
        let entry = unsafe { self.get_slice_for_coords(group, orient)[36..].as_chunks_unchecked::<2>() };
        let first = [group.into(), orient.into()];
        let (i, representative) = Some(&first).into_iter().chain(entry.iter()).enumerate().min_by_key(|(_, x)| *x).unwrap();
        (representative[0].into(), representative[1].into(), SubGroupTransform(i as u8))
    }
}
...
fn test() -> Result<()> {
    ...
    itertools::iproduct!(0..2048, 0..495).par_bridge().for_each(|(i, j)| {
        ...
        for mv in Move::all_iter() {
            ...
            let cubie_moved = (EdgeGroupCoord::from_cubie(moved), EdgeOrientCoord::from_cubie(moved));
        ...
        for transform in SubGroupTransform::all_iter() {
            ...
            let cubie_conjugated = (EdgeGroupCoord::from_cubie(moved), EdgeOrientCoord::from_cubie(moved));
            let table_conjugated = table.conjugate_by_transform(group_coord, orient_coord, transform);
...
fn test_random() -> Result<()> {
    ...
    let table = load_edge_group_and_orient_move_table("edge_group_and_orient_move_table.dat")?;
    itertools::iproduct!(0..2048, 0..495).map(|(i, j)| (i, j, rng.random_range(0..2187))).par_bridge().for_each(|(i, j, k)| {
        let orient_coord = i.into();
        let group_coord = j.into();
