...
use crate::{
    coords::SymEdgeGroupFlipCoord, moves::Move, symmetries::SubGroupTransform,
    tables::table_loader::as_u16_slice_mut,
};

use super::{
    lookup_sym_edge_group_flip::LookupSymEdgeGroupFlipTable,
    move_raw_edge_group_flip::MoveRawEdgeGroupFlipTable,
...
pub struct MoveSymEdgeGroupFlipTable(Mmap);

impl MoveSymEdgeGroupFlipTable {
    pub fn apply_move(
        &self,
        coord: SymEdgeGroupFlipCoord,
        mv: Move,
    ) -> (SymEdgeGroupFlipCoord, SubGroupTransform) {
    ...
    fn generate(
        buffer: &mut [u8],
        sym_lookup_table: &LookupSymEdgeGroupFlipTable,
        move_table: &MoveRawEdgeGroupFlipTable,
    ) {
