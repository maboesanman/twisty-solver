...
use crate::{
    ...
    permutation_coord::{is_odd, is_perm, permutation_coord_12_inverse, permutation_coord_8_inverse},
...
impl ReprCube {
    ...
    pub const fn is_valid(mut self) -> bool {
        let mut sum = 0;
        let mut i = 0;
        while i < 12 {
    ...
    pub fn all_nontrivial_subgroup_conjugations(self) -> impl Iterator<Item = (SubGroupTransform, Self)> {
        (1..16).into_iter().map(SubGroupTransform).map(move |t| (t, self.conjugate_by_subgroup_transform(t)))
    }
    ...
    pub fn phase_1_move_table_entry_cubes(self) -> impl Iterator<Item = Self> {
        Move::all_iter().map(move |m| self.then(m.into())).chain(
            (1..16).into_iter().map(SubGroupTransform).map(move |t| self.conjugate_by_subgroup_transform(t))
        )
    }

    /// returns exactly 10 + 15 = 25 items.
    pub fn phase_2_move_table_entry_cubes(self) -> impl Iterator<Item = Self> {
        Phase2Move::all_iter().map(move |m| self.then(m.into())).chain(
            (1..16).into_iter().map(SubGroupTransform).map(move |t| self.conjugate_by_subgroup_transform(t))
        )
    }
}
...
impl Distribution<ReprCube> for StandardUniform {
    fn sample<R: rand::Rng + ?Sized>(&self, rng: &mut R) -> ReprCube {
        ...
        for i in 0..11 {
            ...
            sum += val;
            edge_orient_coord = edge_orient_coord >> 1;
        ...
        for i in 0..7 {
            ...
            sum += val;
            corner_orient_coord = corner_orient_coord / 3;
