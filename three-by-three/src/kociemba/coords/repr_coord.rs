use crate::{
    ...
    kociemba::{
        coords::{
            coords::{
                CornerOrientRawCoord, CornerPermSymCoord, EEdgePermRawCoord,
                EdgeGroupOrientRawCoord, EdgeGroupOrientSymCoord, UDEdgePermRawCoord,
            },
...
// none of these coords is ever u16::MAX so there are sentinels available
...
impl SymReducedRepr {
    ...
    pub fn unpack(self) -> Phase1Unpacked {
        self.unpack_phase_1()
    }

    pub fn into_cube(self, tables: &Tables) -> ReprCube {
        self.unpack().into_cube(tables)
    }
    ...
    pub fn from_cube(cube: ReprCube, tables: &Tables) -> Self {
        Phase1Unpacked::from_cube(cube, tables).pack(tables).0
    }
}
...
impl Phase2Unpacked {
    ...
    pub fn pack(self, tables: &Tables) -> SymReducedReprPhase2 {
        let norm = self.normalize(tables);
...
mod tests {
    ...
    //         let b = Phase1Unpacked::from_cube(a, &tables).pack(&tables).0;
