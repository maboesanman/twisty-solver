...
use crate::{
    ...
    tables::table_loader::{as_u32_slice, collect_unique_sorted_parallel}, Tables,
};
...
impl LookupSymEdgeGroupOrientTable {
    ...
    pub fn get_all_stabilizing_conjugations(
        ...
        let rep = tables.lookup_sym_edge_group_orient.get_rep_from_sym(sym_coord);
...
mod test {
    ...
    fn test_stabilizing_conjugations() -> anyhow::Result<()> {
        ...
        (0..64430).into_par_iter().for_each(|i| {
            ...
            let group_orient = EdgeGroupOrient::from_coord(rep);
            let stabilizing_conjugations = LookupSymEdgeGroupOrientTable::get_all_stabilizing_conjugations(sym, &tables).into_iter().collect_vec();
